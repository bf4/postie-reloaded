...
/**
  * This file contains modifications by Greg Medding in the attachment handling
  * and category SQL queries.  The category code could easily be modified to
  * allow for tag assignment.
  * 3gp and image attachments now have a <p> tag after them to improve placement
  * in the theme I use, but may be undesireable in other themes...
  * Some addition modifications by Benajmin Fleischer at BF-CHANGED markers also in 
  * get_mail.php, added footer.inc.php  header.inc.php */
  
if(isset($_POST['post_content'])){
$post_categories = GetPostCategories($_POST["post_title"]);
$details = array(
        ...
        'post_modified'		=> $_POST["post_date"],
        'post_modified_gmt'	=> $_POST["post_date_gmt"],
        'ping_status' => $_POST["ping_status"],
        'post_category' => $post_categories,
        'comment_status' => $_POST["comment_status"],
        'post_name' => $_POST["post_name"],
        'post_status' => $config["POST_TYPE"],  //or hardcode to publish or draft
        'tags_input' => $_POST["tags_input"]	// Should be possible to replace this with something similar to the post_category
    );
PostToDB(stripslashes($details)); 
}
...
// replace emails and links that are identifiable BF-CHANGED
...
if(PURIFIER == "on") {
$purifier_ref =& $purifier;
$content = addslashes(getpurified()->purify($content));
} else {
$content = addslashes($content);
    ...
    $details = array(
        ...
        'post_content'		=> $content, //added addslashes BF-CHANGED
        'post_title'		=> addslashes($subject),
        ...
        'post_status' => $config["POST_TYPE"], //bf-changed
//match email address \b[A-Z0-9._%+-]+@(?:[A-Z0-9-]+\.)+[A-Z]{2,4}\b
        'tags_input' => $config["DEFAULT_TAG"]	// Should be possible to replace this with something similar to the post_category
    );

    DisplayEmailPost($details, $config["AJAX_SELECT"]);
if($config["AJAX_SELECT"]=="0") {
    PostToDB($details); 
//print 'AJAX0'.$config["AJAX_SELECT"];
		} 
...
function IMAPMessageFetch ($config = NULL ) {			
	...
	for ($i=1; $i <= $msg_count; $i++) {
		...
		//	$mail_server->deleteMessage($i); // BF-CHANGED
		}
        else {
    ...
    if ( $config["DELETE_MAIL_AFTER_PROCESSING"]) {
        //$mail_server->expungeMessages(); //BF-CHANGED
    }
	//clean up
...
echo 'result'.$post_ID; //debug postID
...
/*                            '<a href="'.$config["URLFILESDIR"] . $filename.'">Video'.
                            '<object '.
                               ...
                               '</object></a>';
*/
							'<!--Mime Type of File is video/3gpp2 --> '.
							'<object '.
                                'classid="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B" '.
                                'codebase="http://www.apple.com/qtactivex/qtplugin.cab" '.
                                'width="173" '.
                               ...
                               '<embed '.
								...
								'></embed> '.
                               '</object><br>'.
							   '<a href="'.$config["URLFILESDIR"] . $filename.'">Direct Link</a><p>';
...
/*            $sql_name = 'SELECT ' . $wpdb->terms. '.term_id 
						 FROM ' . $wpdb->terms. ' 
						 INNER JOIN ' . $wpdb->term_taxonomy . ' 
						 ON ' . $wpdb->terms. '.term_id=' . $wpdb->term_taxonomy . '.term_id 
						 WHERE ' . $wpdb->terms. '.name=\'' . addslashes($match) . '\' 
            ...
            $sql_sub_name = 'SELECT ' . $wpdb->terms. '.term_id 
							 FROM ' . $wpdb->terms. ' 
							 INNER JOIN ' . $wpdb->term_taxonomy . ' 
							 ON ' . $wpdb->terms. '.term_id=' . $wpdb->term_taxonomy . '.term_id 
							 WHERE ' . $wpdb->terms. '.name LIKE \'' . addslashes($match) . '%\' 
							 AND ' . $wpdb->term_taxonomy . '.taxonomy=\'category\' LIMIT 1';
...
/**
  ...
  */
function DisplayEmailPost($details, $ajax) {
            // Report
            // BF-CHANGED to table
            $rowid = rand();
            print '</pre>';
			print '<tr id="'.$rowid.'" class="normal">';
		if($ajax=="1"){
			print '<td>';
			print '<input type="checkbox" onclick="javascript:AjaxPost('.$rowid.');"></td>';
			} 
			print '<td><form name="'.$rowid.'" id="'.$rowid.'form">';
 print '</pre><p><b>Post Author</b>: <textarea class="small" name="post_author">' . $details["post_author"]. '</textarea><br />' . "\n";
            
            print '<b>Date</b>: <textarea name="post_date">' . $details["post_date"] . '</textarea><br />' . "\n";
            print '<b>Date GMT</b>: <textarea class="small" name="post_date_gmt">' . $details["post_date_gmt"] . '</textarea><br />' . "\n";
            foreach($details["post_categories"] as $category) {
                print '<input type="hidden" class="small" name="post_categories" value="'.$category.'"><b>Category</b>: ' . $category . '<br />' . "\n";
            }
            print '<b>Ping Status</b>: <textarea class="small"  name="ping_status">' . $details["ping_status"] . '</textarea><br />' . "\n";
            print '<b>Comment Status</b>: <textarea class="small" name="comment_status">' . $details["comment_status"] . '</textarea><br />' . "\n";
            print '<b>Subject</b>: <textarea class="small" name="post_title">' . $details["post_title"]. '</textarea><br />' . "\n";
			print '<p><b>Tags</b>:  <textarea class="small" name="tags_input">'. $details["tags_input"] . '</textarea><br />' . "\n";
            print '<b>Postname</b>: <textarea class="small" name="post_name">' . $details["post_name"] . '</textarea><br />' . "\n";
            print '<b>Posted content:</b></p><textarea class="big" name="post_content">' . $details["post_content"] . '</textarea></form><hr /><hr /></td></tr><pre>';
}
/**
...
function GetDBConfig() {
    ...
    //if (!isset($config["DEFAULT_POST_CATEGORY"])) { $config["DEFAULT_POST_CATEGORY"] =  get_option('default_category'); }
...
function GetConfig() {
    ...
    $config["DELETE_MAIL_AFTER_PROCESSING"] = true; //no delete change to false BF-CHANGED
...
if (!isset($config["AJAX_SELECT"])) { $config["AJAX_SELECT"] = "0";} // BF-CHANGED
else {
	 if (!current_user_can('config_postie') && $config["AJAX_SELECT"] == "1") {  die("Must have admin rights to use ajax"); }
}
if (!isset($config["POST_TYPE"])) { $config["POST_TYPE"] = "publish";} // BF-CHANGED was draft
if (!isset($config["DEFAULT_TAG"])) { $config["DEFAULT_TAG"] = DEFAULT_TAGS;}
    return $config;
}
/**
