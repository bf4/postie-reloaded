        ...
        <legend><?php _e('Message Settings');?></legend> 
            ...
            <tr valign="top">
                ...
                <td><select name="DEFAULT_POST_CATEGORY" id="DEFAULT_POST_CATEGORY">
                        <?php
                        //$categories = $wpdb->get_results("SELECT $wpdb->terms.* FROM $wpdb->terms INNER JOIN $wpdb->term_taxonomy ON $wpdb->terms.term_id=$wpdb->term_taxonomy.term_id WHERE $wpdb->term_taxonomy.taxonomy='category' ORDER BY name");
...
<tr><td colspan=2>
<fieldset class="options">
<legend><?php _e('Use Ajax to Approve Posts');?></legend>
<table width="100%" cellspacing="2" cellpadding="5" class="editform">
<th scope="row"><?php _e('Ajax Approve:') ?></th><td><?php echo BuildBooleanSelect("Use Ajax Approve?","AJAX_SELECT",$config["AJAX_SELECT"],"This controls if the Ajax is enabled to allow you to individually approve posts");?>
 ...
 <tr> 
                <th width="33%" valign="top" scope="row"><legend><?php _e('Prefered Post Type (Publish/Draft):') ?></legend> </th> 
                <td>
        <select name="POST_TYPE" id="POST_TYPE">
        <option value="draft">save as draft</option>
        <option value="publish" <?php if($config["POST_TYPE"] == "publish") { echo "SELECTED";} ?>>publish</option>
        </select>
                <br />
                </td> 
            </tr> 
<tr> 
                <th width="33%" valign="top" scope="row"><legend><?php _e('Default Post Tag(s):') ?></legend> </th> 
                <td>
                <input name="DEFAULT_TAG" type="text" id="DEFAULT_TAG" value="<?php echo $config["DEFAULT_TAG"]; ?>" size="50" /><br />
                <?php _e("Recommended");?>: <code>Mobile</code>
                <br />
                </td> 
            </tr> 
    <tr><td colspan=2> 
